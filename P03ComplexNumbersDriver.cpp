...
int main()
{
    ...
    cout<<"\n\nTesting for addition"<<endl;
    assertx(z1 + 3.0 == complex(z1.real()+3.0, z1.imag()), __LINE__);

    cout <<"\nTesting for subtraction"<<endl;
    assertx(z1 - 3.0 == complex(z1.real() -3.0, z1.imag()), __LINE__);

    cout<<"\nTesting for multiplication"<<endl;
    assertx(z1 * 4.0 == complex(z1.real()*4.0, z1.imag()*4.0), __LINE__);

    cout<<"\nTesting for  division"<<endl;
    assertx(z1 / 3.0 == complex(z1.real()/3.0, z1.imag()/3.0), __LINE__);

    cout<<"\nTesting for bool "<<endl;
    assertx(z2 == z2, __LINE__);
    assertx(z2 != z3, __LINE__);

cout<<"\nTest copy constructor"<<endl;
    complex test2(z4);
    assertx(test2.real() == z4.real() && test2.imag() == z4.imag(), __LINE__);

    cout<<"\nTesting for  conjugate"<<endl;
    assertx(conj(z2) == complex(z2.real(), -z2.imag()), __LINE__);

    cout<<"\nTesting for norm"<<endl;
    assertx(norm(z2) == z2.real()*z2.real() + z2.imag()*z2.imag(), __LINE__);

//    cout<<"\nTesting for default Constructor"<<endl;
//    assertx(z5.real() == 1.1 && z5.imag() == 1.1,__LINE__);

cout<<"\nTesting the instream operator with (5,3): ";
complex result1;

    cin>> result1;
    assertx(result1.real() == 5.0,__LINE__);
    assertx(result1.imag() == 3.0,__LINE__);

    cout<<"ostream operator for the (5,3) = "<< result1<<endl;
